...
//                this.whiteNoise.start(0);
        ...
        this.onCharacterPlay = function (c,s) {
                ...
                if (stop_flag) {
                    ...
                    //txt = c["c"].replace(/　/g, '');
                    textModMsg.innerText = textModMsg.innerText+c["c"];
                    console.log(stop_flag);
                }
        ...
        this.renderPlayer = function(el, obj) {
            ...
            btn_yomiage.onclick = function () {
                ...
                var result3 = text.replace(/ヌナヌナメ\r?\n/g, 'ほんぶん.　　').replace(/ホレメ２\r?\n/g, 'にまいめいきます.　　').replace(/゛゛\r?\n/g, 'つぎのぎょう.　　').replace(/ロ/g, '路上歩行.　　').replace(/ホ/g, '報告.　　').replace(/ヨ/g, '要綱.　　').replace(/レ/g, '礼装用.　　').replace(/ツ/g, '都合どうか.　　').replace(/ナ/g, 'なろおた.　　').replace(/ラ/g, 'ラムネ.　　').replace(/ノ/g, '乃木東郷.　　').replace(/フ/g, '封筒貼る.　　').replace(/キ/g, '聞いて報告.　　');
                console.log(result3);
